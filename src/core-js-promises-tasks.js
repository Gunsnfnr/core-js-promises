...
function getAllResult(promises) {
  const result = [];
  const allResults = Promise.allSettled(promises).then((value) => {
    value.forEach((val) => {
      const currentVal = val.value || null;
...
// console.log('JSON.parse(values): ', JSON.parse(values));
...
function queuPromises(promises) {
  const resultingPromise = new Promise(function handlePromises(resolve) {
    const sum = [];
    let total = 0;
    promises.forEach((item, index) => {
      Promise.resolve(item).then((result) => {
        sum[index] = result;
        total += 1;
        if (total === promises.length) resolve(sum.join(''));
      });
    });
  });
